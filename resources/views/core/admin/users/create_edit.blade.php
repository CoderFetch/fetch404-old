		...
		<div class="tab-content">
			...
			<div class="tab-pane active" id="tab-general">
				...
				<div class="form-group {{{ $errors->has('name') ? 'error' : '' }}}">
					...
					<div class="col-md-10">
						<input class="form-control" type="text" name="name" id="username" value="{{{ Input::old('name', isset($user) ? $user->name : null) }}}" />
				...
				<div class="form-group {{{ $errors->has('activated') || $errors->has('confirm') ? 'error' : '' }}}">
					...
					<div class="col-md-6">
						...
						@else
							<select class="form-control" {{{ ($user->id === Auth::id() ? ' disabled="disabled"' : '') }}} name="confirm" id="confirm">
								<option value="1"{{{ ($user->confirmed ? ' selected="selected"' : '') }}}>Yes</option>
								<option value="0"{{{ ( ! $user->confirmed ? ' selected="selected"' : '') }}}>No</option>
							</select>
						@endif
