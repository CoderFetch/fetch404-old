	...
	public function serverPlay($server_ip, $server_port, $server_name){
		...
		if($Info['players']['online'] == 0) {
			...
			for ($row = 0; $row < $max; $row++) {
				echo '<span data-type="tooltip" data-placement="bottom" data-original-title="'.$Info['players']['sample'][$row]['name'].'"><img src="https://cravatar.eu/avatar/' .$Info['players']['sample'][$row]['name'] . '/50.png" style="width: 40px; height: 40px; margin-bottom: 7px; margin-left: 5px; border-radius: 5px;" /></span>';
			}
			if(isset($extra)){
